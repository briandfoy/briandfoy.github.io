...
So, let's summarize the book. It's really easy because most of the text is the sort of fluff that has taken over these sorts of non-fiction books where the author is a major part of the story. It's entertaining when Hunter Thompson does it, but no one is Hunter Thompson, so stop it.
...
* Some of the confusion is that an assigned number, no matter the system, corresponds to physical space. That's one way to do it, but imagine a completely different system now where every object is easily discovered by RFID and is randomly shelved. When that book is requested, the RFID system locates it and presents it. Consider the difference in grocery stores before and after [Piggly Wiggly let shoppers collect their own groceries](https://www.smithsonianmag.com/smart-news/bizarre-story-piggly-wiggly-first-self-service-grocery-store-180964708/) directly off the shelves. In the before times, the shopper didn't need to know how to find the item, and in the after times, they did.
