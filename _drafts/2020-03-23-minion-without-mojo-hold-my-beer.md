...
The basic idea is that I can throw some work out into the universe and something else can do it while my program moves on to the next thing. My frontline programs are simpler because they merely note that something needs to be done.

For example, my web app might accept a file upload. However, the web app doesn't need to process the file—something else does that later. The web app doesn't care because it needs to accept files from other requests. It adds the new file to a queue and trusts that the processing will eventually happen.
...
For my demonstration, I want to use the Postgres backend so remote workers can connect to it to get some jobs. Install Postgres, configure it for remote access, and create a database. I used `minion` but name it whatever you like. I don't need to create tables or anything else. Minion will do that for me the first time it connects.
